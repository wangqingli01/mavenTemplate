...
public class StudentUtil extends MyDatabaseUtil implements StudentModel, StudentRedis {
    ...
    private static final List<String> STUDENT_Status = Arrays.asList("在籍", "非在籍", "退学");
    ...
    public static boolean isValidStuSchoolName(String schoolName) {
        return isValidUUID(schoolName);
    }
    
    public static boolean isValidStuNation(String nation) {
        return isValidUUID(nation);
    }
    
    public static boolean isValidStuIDNumber(String idNumber) {
        return isValidUUID(idNumber);
    }
    public static boolean isValidStuGradation(String gradation) {
        return isValidUUID(gradation);
    }
    public static boolean isValidStuLength(String length) {
        return isValidUUID(length);
    }
    public static boolean isValidStuDegreeCategory(String degreeCategory) {
        return isValidUUID(degreeCategory);
    }
    public static boolean isValidStuDegreeForm(String degreeForm) {
        return isValidUUID(degreeForm);
    }
    public static boolean isValidStuEnrollmentDate(Date enrollmentDate) {
        return true;
    }
    public static boolean isValidStuEstimatedDate(Date estimatedDate) {
        return true;
    }
    public static boolean isValidStuStatus(String status) {
        return STUDENT_Status.contains(status);
    }
    ...
    public static boolean isValidStudentAllInfo(Student student) {
        return !isNull(student) && isValidStuId(student.getStuId()) && isValidStuClass(student.getStuClass())
                ...
                && isValidStuPhone(student.getStuPhone()) && isValidStuRemark(student.getStuRemark())&& isValidStuSchoolName(student.getStuSchoolName()) 
                &&isValidStuNation(student.getStuNation())&&isValidStuIDNumber(student.getStuIDNumber()) &&isValidStuGradation(student.getStuGradation())
                &&isValidStuLength(student.getStuLength())&&isValidStuDegreeCategory(student.getStuDegreeCategory())&&isValidStuDegreeForm(student.getStuDegreeForm())
                &&isValidStuEnrollmentDate(student.getStuEnrollmentDate())&&isValidStuEstimatedDate(student.getStuEstimatedDate())&&isValidStuStatus(student.getStuStatus());
    }
    ...
    public static boolean isValidStudentUpdateInfo(StudentWithGradeClassMajorCollegeDto stuWGCMC) {
        return !isNull(stuWGCMC) && GradeUtil.isValidGradeName(stuWGCMC.getStuGradeName()) && ClassUtil.isValidClassName(stuWGCMC.getStuClassName())
                ...
                && StudentUtil.isValidStuDegree(stuWGCMC.getStuDegree()) && StudentUtil.isValidStuPhone(stuWGCMC.getStuPhone()) 
                && StudentUtil.isValidStuRemark(stuWGCMC.getStuRemark())&& StudentUtil.isValidStuSchoolName(stuWGCMC.getStuSchoolName())
                && StudentUtil.isValidStuNation(stuWGCMC.getStuNation())&& StudentUtil.isValidStuIDNumber(stuWGCMC.getStuIDNumber())
                && StudentUtil.isValidStuGradation(stuWGCMC.getStuGradation())&& StudentUtil.isValidStuLength(stuWGCMC.getStuLength())
                && StudentUtil.isValidStuDegreeCategory(stuWGCMC.getStuDegreeCategory())&& StudentUtil.isValidStuDegreeForm(stuWGCMC.getStuDegreeForm())
                && StudentUtil.isValidStuEnrollmentDate(stuWGCMC.getStuEnrollmentDate())&& StudentUtil.isValidStuEstimatedDate(stuWGCMC.getStuEstimatedDate())
                && StudentUtil.isValidStuStatus(stuWGCMC.getStuStatus())
                ;
    }
