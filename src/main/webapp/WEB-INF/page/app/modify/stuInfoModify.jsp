        ...
        form.on('submit(LAY-app-contcomm-search)', function (data) {
            ...
            table.reload('stuInfoQuery', {
                ...
                , where: { //设定异步数据接口的额外参数，任意设
                    ...
                    , stuSchoolName:field.stuSchoolName
                    , stuNation:field.nation
                    , stuIDNumber:field.idNumber
                    , stuLength:field.gradation
                    , stuGradation:field.Length
                    , stuDegreeCategory:field.degreeCategory
                    , stuDegreeForm:field.degreeForm
                    , stuEnrollmentDate:field.enrollmentDate
                    , stuEstimatedDate:field.estimatedDate
                    , stuStatus:field.status
                }
                , request: {
                    pageName: 'pageNum',
                    limitName: 'pageSize'  //如不配置，默认为page=1&limit=10
            ...
            if (obj.event === 'del') {
                ...
                var index = layer.open({
                    ...
                    yes: function (index, layero) {
                        ...
                        iframeWindow.layui.form.on('submit(layuiadmin-app-form-edit)', function (data) {
                            ...
                            var json = {
                                ...
                                ,stuNation:field.nation
                                ,stuIDNumber:field.idNumber
                                ,stuLength:field.Length
                                ,stuGradation:field.gradation
                                ,stuDegreeCategory:field.degreeCategory
                                ,stuDegreeForm:field.degreeForm
                                ,stuEnrollmentDate:field.enrollmentDate
                                ,stuEstimatedDate:field.estimatedDate
                    ...
                    success: function (layero, index) {
                        ...
                        othis.find('input[name="schoolName"]').val(data.stuSchoolName);
                        othis.find('input[name="nation"]').val(data.stuNation);
                        othis.find('input[name="idNumber"]').val(data.stuIDNumber);
                        othis.find('input[name="Length"]').val(data.stuLength);
                        othis.find('input[name="gradation"]').val(data.stuGradation);
                        othis.find('input[name="degreeCategory"]').val(data.stuDegreeCategory);
                        othis.find('input[name="degreeForm"]').val(data.stuDegreeForm);
                       /*  othis.find('input[name="enrollmentDate"]').val(data.stuEnrollmentDate);
                        othis.find('input[name="estimatedDate"]').val(data.stuEstimatedDate);
                        othis.find('input[name="status"]').val(data.stuStatus); */
                        othis.find('input[name="status"][value="在籍"]').attr("checked", data.stuStatus == '在籍' ? true : false);
                        othis.find('input[name="status"][value="非在籍"]').attr("checked", data.stuStatus == '非在籍' ? true : false);
                        othis.find('input[name="status"][value="退学"]').attr("checked", data.stuStatus == '退学' ? true : false);
